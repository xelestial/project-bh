...
test("http server supports room lifecycle and websocket lobby broadcast", async (context) => {
  ...
  try {
    ...
    await waitForMessage<{ type: string }>(socket);
    ...
    const lobbyUpdate = await waitForMessage<{
    ...
    const startedRoom = await fetch(`${baseUrl}/api/rooms/${createPayload.room.roomId}`);
